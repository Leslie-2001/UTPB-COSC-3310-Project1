...
public class UInt {
    ...
    public void add(UInt u) {
        ...
        if(this.length > u.length) {
            ...
            uTemp.addZerosToBinary(diffInLengths);
        }
        else { // (this.length < u.length)
            diffInLengths = u.length - this.length;
            this.addZerosToBinary(diffInLengths);
        }
    ...
    public void bypassLastCarry(UInt u) {
        int trueCount = 0;
        int diffInLengths;
        ...
        if(this.length > u.length) {
            diffInLengths = this.length - u.length;
            u.addZerosToBinary(diffInLengths);
        }
        else { // (this.length < u.length)
            diffInLengths = u.length - this.length;
            this.addZerosToBinary(diffInLengths);
    ...
    public static UInt bypassLastCarry(UInt a, UInt b) {
        UInt temp = a.clone();
        temp.bypassLastCarry(b);
        return temp;
    }
    ...
    public void negate() {
        ...
        this.bypassLastCarry(new UInt(1));
    }
    ...
    public void sub(UInt u) {
        ...
        // pad the shorter array with leading false(s)/zeroes
        if(this.length > u.length) {
            lengthDiff = this.length - u.length;
            tempU.addZerosToBinary(lengthDiff);
        }
        else { // (this.length < u.length)
            lengthDiff = u.length - this.length;
            this.addZerosToBinary(lengthDiff);
        }

        tempU.negate();

        // Add the two together using add()
        this.bypassLastCarry(tempU);
    ...
    public void mul(UInt u) {
        ...
        if(m.length > r.length) {
            lengthDiff = m.length - r.length;
            r.addZerosToBinary(lengthDiff);
        }
        else { // (this.length < u.length)
            lengthDiff = r.length - m.length;
            m.addZerosToBinary(lengthDiff);
            m_negated.addZerosToBinary(lengthDiff);
        }
    ...
    public void addZerosToBinary(int numZeroes) {
